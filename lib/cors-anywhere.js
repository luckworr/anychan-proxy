...
function setResponseHeaders(headers, request) {
  ...
  if (config.cookies) {
    if (!config.allowedRequestOrigins) {
    ...
    if (!config.shareCookiesBetweenAllowedRequestOrigins) {
  ...
  if (config.allowedRequestOrigins) {
    headers['access-control-allow-origin'] = getAllowedRequestOrigin(config.allowedRequestOrigins, request);
  } else {
    headers['access-control-allow-origin'] = '*';
  }
