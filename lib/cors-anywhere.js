...
function onProxyResponse(proxy, proxyReq, proxyRes, req, res) {
  ...
  // Add `x-cookies` header that could be used to debug
  // the actual cookies being sent to the server.
  if (req.areCookiesAllowed) {
    proxyRes.headers['x-cookies'] = req.headers['cookies'];
  }
...
function getHandler(options, proxy) {
  ...
  return function(req, res) {
    ...
    // If `x-cookies` request header is specified,
    // add the cookies from it to the `cookies` header.
    if (req.areCookiesAllowed) {
      if (req.headers['x-cookies']) {
        if (req.headers['cookies']) {
          req.headers['cookies'] = req.headers['cookies'] + '; ' + req.headers['x-cookies'];
        } else {
          req.headers['cookies'] = req.headers['x-cookies'];
        }
      }
    }
